...
Platoon = Class(RNGAIPlatoon) {
    ...
    EngineerBuildAIRNG = function(self)
        ...
        if self.PlatoonData.DoMexCap then
            if not aiBrain.mexcapplatoons then aiBrain.mexcapplatoons={} end
            table.insert(aiBrain.mexcapplatoons,self)
        end
        local eng
        for k, v in platoonUnits do
        ...
        elseif cons.OrderedTemplate then
            ...
            relative = true
            local tmpReference = aiBrain:FindPlaceToBuild('T3EnergyProduction', 'uab1201', baseTmplDefault['BaseTemplates'][factionIndex], relative, eng, nil, relativeTo[1], relativeTo[3])
    ...
    TrueHeroRNG = function(self)
        ...
        while PlatoonExists(aiBrain, self) do
            ...
            if not target or target.Dead or not RUtils.CanGraphToRNGArea(Hero,targetPosition,self.MovementLayer) then
                Hero.target=nil
                ...
                for _,v in mex do
                    ...
                    if not RUtils.CanGraphToRNGArea(Hero,v.Position,self.MovementLayer) then
                        continue
                    end
                ...
                if table.getn(raidlocs)==0 then WaitSeconds(3) continue end
    ...
    CHPMergePlatoon = function(self,radius)
        ...
        for i, p in ps1 do
            if not p or p==self or not aiBrain:PlatoonExists(p) or not p.chpdata.name or not (p.chpdata.name==self.chpdata.name) or not p.MovementLayer==self.MovementLayer or VDist3Sq(self:GetPlatoonPosition(),p:GetPlatoonPosition())>best or table.getn(p:GetPlatoonUnits())>30 then  
                --LOG('merge table removed '..repr(i)..' merge table now holds '..repr(table.getn(ps)))
            else
                best=VDist3Sq(self:GetPlatoonPosition(),p:GetPlatoonPosition())
                table.insert(ps,p)
            end
        end
    ...
    TruePlatoonRNG = function(self)
        ...
        self:ForkThread(self.PriorityHandler)
        ...
        while PlatoonExists(aiBrain, self) do
            ...
            if target then
                ...
                if not RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),targetPosition,self.MovementLayer) then
                    target=nil
                    platoon.target=nil
                    targetPosition=nil
                end
            end
            if platoon.rdest and RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),platoon.rdest,self.MovementLayer) and VDist3Sq(platoon.Pos,platoon.rdest)>(platoon.MaxWeaponRange+20)*(platoon.MaxWeaponRange+20) then
            ...
            if not target and not targetacu or targetDist>math.max(platoon.MaxWeaponRange,40) or (not target and targetacuDist>platoon.MaxWeaponRange*3) or target and not RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),targetPosition,self.MovementLayer) then
                ...
                if target then
                    ...
                    if not platoon.path then 
                        ...
                        for _,v in mex do
                            ...
                            if not RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),v.Position,self.MovementLayer) then
                                continue
                            end
                        ...
                        if VDist3Sq(platoon.Pos,platoon.dest)<40*40 then
                            for _,v in platoonUnits do
                                if not v or v.Dead then continue end
                                IssueClearCommands({v})
                                IssueMove({v},RUtils.LerpyRotate(v:GetPosition(),platoon.dest,{VDist3(v:GetPosition(),platoon.dest),math.random(0,20)}))
                                WaitTicks(1)
                                continue
                            end
                            WaitTicks(20)
                ...
                elseif platoon.path then
                    if VDist3Sq(platoon.Pos,platoon.path[table.getn(platoon.path)])<40*40 then
                        for _,v in platoonUnits do
                            if not v or v.Dead then continue end
                            IssueClearCommands({v})
                            IssueMove({v},RUtils.LerpyRotate(v:GetPosition(),platoon.dest,{VDist3(v:GetPosition(),platoon.path[table.getn(platoon.path)]),math.random(0,20)}))
                            WaitTicks(1)
                            continue
                        end
                        WaitTicks(20)
                    ...
                    for _,v in mex do
                        ...
                        if not AIAttackUtils.GetMostRestrictiveLayer(platoon) or not RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),v.Position,self.MovementLayer) then
                            continue
                        end
                        table.insert(raidlocs,v)
                    end
                ...
                if platoon.enemyThreat<platoon.ThreatLimit/2 and not platoon.Sniper and not targetacuDist<40 or platoon.enemyThreat<platoon.ThreatLimit/5 then
                    ...
                    if targetDist<platoon.MaxWeaponRange*1.5 then
                        ...
                        for _,v in platoonUnits do
                            ...
                            if v.Support and VDist3Sq(v:GetPosition(),platoon.Pos)>8*8 then
                                IssueClearCommands({v})
                                IssueMove({v},RUtils.lerpy(platoon.dest,platoon.home,{VDist3(platoon.home,platoon.dest),20}))
                                --WaitTicks(1)
                                continue
                            end
    ...
    HighlightTruePlatoon = function(self)
        ...
        while not platoon.dead and PlatoonExists(aiBrain, self) do
                ...
                if platoon.raid and platoon.rdest then
                    DrawLinePop(pos1,platoon.rdest,'ffFFFFFF')
                end
    ...
    PathNavigationRNG = function(self)
        ...
        while not platoon.dead and PlatoonExists(aiBrain, self) do
            ...
            if not platoon.path or VDist3Sq(platoon.path[table.getn(platoon.path)],platoon.Pos)<20*20 then platoon.pathretreat=nil platoon.navigating=nil WaitTicks(20) continue end
            if platoon.navigating then
                ...
                if not enemy or enemy.Dead then
                    ...
                    if RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),enemy:GetPosition(),self.MovementLayer) and (VDist3Sq(enemy:GetPosition(),self:GetPlatoonPosition())<math.max(platoon.MaxWeaponRange*platoon.MaxWeaponRange*3,400) and not platoon.raid or (VDist3Sq(enemy:GetPosition(),self:GetPlatoonPosition())<400) ) then
            ...
            if (platoon.dest and not RUtils.CanGraphToRNGArea(AIAttackUtils.GetMostRestrictiveLayer(platoon),platoon.dest,self.MovementLayer)) or (platoon.path and GetTerrainHeight(platoon.path[table.getn(platoon.path)][1],platoon.path[table.getn(platoon.path)][3])<GetSurfaceHeight(platoon.path[table.getn(platoon.path)][1],platoon.path[table.getn(platoon.path)][3])) then
                platoon.navigating=false
                platoon.path=nil
                WaitTicks(20)
                continue
    ...
    MexBuildAIRNG = function(self)
        ...
        local eng=platoonUnits[1]
        if not eng.ProcessBuild then eng.ProcessBuild=true end
        if eng.platoonactive then return end
        eng.platoonactive=true
        self:Stop()
        if not eng or eng.Dead then
            WaitTicks(1)
            self:PlatoonDisband()
        ...
        while not aiBrain.expansionMex do DrawCircle(eng:GetPosition(),3,'FFFFFF00') WaitTicks(2) end
        local markerTable=table.copy(aiBrain.expansionMex)
        if eng.Dead then self:PlatoonDisband() end
        for i,v in markerTable do
            if not RUtils.CanGraphToRNGArea(eng,v.Position,'Amphibious') then table.remove(markerTable,i) end
        end
        while eng and not eng.Dead do
            ...
            eng.mexesqueued=0
            for i,v in markerTable do
                if aiBrain:CanBuildStructureAt(whatToBuild, v.Position) then
                    if v.Position then
            ...
            if not currentmexpos then IssueClearCommands({eng}) continue end
            ...
            for _=0,3,1 do
                ...
                if (bool and finalqueued<2) or (bool and eng.mexesqueued<2) then
                    ...
                    for _,massMarker in markers do
                        if (finalqueued>=4) or (eng.mexesqueued>=2 and finalqueued>=2) or (eng.mexesqueued>=4) then break end
            ...
            if eng.mexesqueued<1 then IssueClearCommands({eng}) continue end
            WaitTicks(2)
            while eng and not eng.Dead and ( not eng.Dead and not eng:IsIdleState() or eng:GetCommandQueue() and table.getn(eng:GetCommandQueue())>0 or eng:IsUnitState('Building') or eng:IsUnitState("Moving")) do
                WaitTicks(2)
            end
            LOG(repr(eng.EngineerBuildQueue))
            eng.EngineerBuildQueue={}
            IssueClearCommands({eng})
            WaitTicks(20)
    ...
    EngineerTemporaryCircleRender = function(eng,pos,color,rad)
        for _=0,10 do
            for _=0,5 do
