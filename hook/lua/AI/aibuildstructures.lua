...
function AIExecuteBuildStructureRNG(aiBrain, builder, buildingType, closeToBuilder, relative, buildingTemplate, baseTemplate, reference, constructionData)
    ...
    if IsResource(buildingType) then
        if buildingType ~= 'T1HydroCarbon' and constructionData.TakeClose then
            local markerTable=table.copy(aiBrain.cmanager.soonmexes)
            relative = false
            ...
            --for i,v in markerTable do
                --if aiBrain:CanBuildStructureAt('ueb1103', v.Position) then
                    ...
                    location = table.copy(markerTable[math.random(1,table.getn(markerTable))])
                ...
                --end
            --end
        elseif buildingType ~= 'T1HydroCarbon' and constructionData.PriorityExpand then
            ...
            table.sort(markerTable,function(a,b) return VDist2Sq(a.Position[1],a.Position[3],relativeTo[1],relativeTo[3])/a.priority/a.priority*a.distsq*a.distsq<VDist2Sq(b.Position[1],b.Position[3],relativeTo[1],relativeTo[3])/b.priority/b.priority*b.distsq*b.distsq end)
            for i,v in markerTable do
                if VDist3Sq( v.Position, relativeTo ) <= constructionData.MaxDistance*constructionData.MaxDistance*2 and VDist3Sq( v.Position, relativeTo ) >= constructionData.MinDistance*constructionData.MinDistance then
                    if aiBrain:CanBuildStructureAt('ueb1103', v.Position) then
                        --[[if v.expand.taken>0 and v.expand.mexnum/(v.expand.taken+1)<4 then 
                            if v.expand.takentime+40>GetGameTimeSeconds() then
                                continue
